...
from data_preprocessing import add_estate_price
...
kakao_api_key = (
    os.getenv("kakao_api_key")  # ✅ 로컬: .env 사용
    or kakao_api_key_by_toml  # ✅ 로컬: secrets.toml 사용
    ...
    for i in range(61):
        ...
        if not df_applicant.empty or not df_applicant.empty:
            df_applicant = df_applicant.sort_values(
                by="연월", ascending=False
            ).reset_index(drop=True)
            df_applicant_list_total = pd.concat(
                [df_applicant_list_total, df_applicant], ignore_index=True
            )
...
def get_estate_detail(ID):
    ...
    for row in table.find("tbody").find_all("tr"):
        ...
        if len(cols) > 0:  # 데이터가 있는 경우
            data_row = [
                np.nan if col.text.strip() == "" else col.text.strip() for col in cols
            ]
    ...
    df_estate_detail = df_estate_detail[
        [
            "주택관리번호",
            "공고번호",
            "주택형",
...
def crawl_naver_news(url, max_articles=3):
    ...
    def crawl_article_content(news_url, headers):
        ...
        try:
            ...
            content_element = article_soup.select_one(
                "#newsct_article"
            ) or article_soup.select_one("#dic_area")
        ...
        try:
            ...
            for item in news_areas:
                ...
                if "청약" in full_content:  # 크롤링된 전체 내용에서 "청약" 키워드 확인
                    articles.append(
                        {"title": title, "content": full_content, "url": news_url}
                    )
...
def get_apartment_news(df):
    ...
    df_unique = df.drop_duplicates(subset="공고번호", keep="first").reset_index(
        drop=True
    )
...
def add_topic_keyword(df_future_estate_list):
    ...
    # 파일 경로를 절대 경로로 변환
    count_vectorizer_path = os.path.join(
        current_dir, "storage/topic_modeling/countvectorizer_model_0.0.1.pkl"
    )
    lda_model_path = os.path.join(
        current_dir, "storage/topic_modeling/lda_model_0.0.1.pkl"
    )
    ...
    cleaned_corpus = [
        extract_nouns(clean_text(text)) for text in corpus
    ]  # 정제 + 명사 추출
    ...
    for i, topic_dist in enumerate(doc_topic):
        ...
        result.update(
            {topic_names[j]: round(topic_dist[j], 4) for j in range(len(topic_names))}
        )
...
def get_lat_lon_kakao(address):
    ...
    try:
        response = requests.get(
            kakao_api_url, headers=headers, params={"query": address}
        )
        # 요청 실패한경우
        if response.status_code != 200:
            return None
...
def process_address_data(address, address_by_apartname, verbose=False):
    ...
    try:
        ...
        if not data:
            if verbose:
                print(
                    f"[주소 변환 실패] '{address}', '{address_by_apartname}' → NaN 값 반환"
                )
        ...
        if verbose:
            print(
                f"[주소 변환 성공] '{address}' → {lat}, {lon}, {h_code}, {b_code}, {region_depth_1}, {region_depth_2}, {region_depth_3_h}, {region_depth_3}"
            )
        ...
        print(
            f"[오류 발생] {e} (address='{address}', address_by_apartname='{address_by_apartname}')"
        )
        return (
            None,
            None,
...
def preprocess_apartname(df):
    ...
    def clean_name(address):
        ...
        cleaned_words = [
            word for word in words if filter_conditions(word)
        ]  # 필터링 적용
...
def add_address_code(df_future_estate_list):
    df_future_estate_list_unique = df_future_estate_list.drop_duplicates(
        subset="공고번호", keep="first"
    )
    ...
    def preprocess_apartname(df):
        ...
        def filter_conditions(word):
            ...
            if word in custom_stopwords:  # 특정 키워드 제거
                return False
            if any(
                sub in word for sub in remove_if_contains
            ):  # 특정 단어 포함 단어 제거
                return False
            return True  # 모든 조건을 통과하면 유지
    ...
    # 주택명으로 공급위치 찾기
    df_future_estate_list_unique["공급위치 by 주택명"] = df_future_estate_list_unique[
        "정제된주택명"
    ].apply(add_address_by_apartname)
    ...
    ] = df_future_estate_list_unique.apply(
        lambda x: process_address_data(x["정제된주소"], x["공급위치 by 주택명"]),
        axis=1,
        result_type="expand",
    )
    df_future_estate_list_unique = df_future_estate_list_unique[
        [
            "공고번호",
            "위도",
            "경도",
    ...
    # 매물 목록에 머지해주기
    df_future_estate_list = pd.merge(
        df_future_estate_list, df_future_estate_list_unique, how="inner"
    )
...
def get_estate_price(estate_id):
    ...
    # DataFrame 생성
    df = pd.read_html(str(table), converters={0: str, 1: str, 2: str})[
        0
    ]  # converters: 각 컬럼의 데이터를 문자열로 변환
    ...
    for estate_id in estate_ids:
        try:
            ...
            for index in range(len(df_estate_price)):
                ...
                estate_price = (
                    float(df_estate_price.loc[index, "공급금액(최고가 기준)"]) * 10000
                )

                mask = (df_future_estate_list["공고번호"] == estate_id) & (
                    df_future_estate_list["주택형"] == estate_type
                )
...
def add_market_profit(df):
    ...
    df["모집공고일_년월"] = (
        pd.to_datetime(df["모집공고일"]).dt.strftime("%Y%m").astype(int)
    )
    ...
    real_estate_price_path = os.path.join(
        current_dir,
        "storage/raw_data/서울경기인천_전체_월별_법정동별_실거래가_평균.csv",
    )
    ...
    def apply_price_diff(row):
        ...
        mask = (df_real_estate_price["법정동코드"] == b_code) & (
            df_real_estate_price["년월"] == date
        )
...
def get_dummy_estate_list():
    ...
    file_path = os.path.join(
        current_dir, "storage/raw_data/병합_청약매물_목록_정보_픽스2.csv"
    )
    # ✅ 파일 존재 여부 확인
    ...
    df.rename(columns={"공급금액(최고가 기준)": "공급금액"}, inplace=True)

    # 기준금리 추가
    df["모집공고일_tmp"] = df["모집공고일"].str[:7]
    df = df.merge(
        df_index, left_on="모집공고일_tmp", right_on="변경일자", how="left"
    ).drop(columns=["변경일자"])
    ...
    filtered_df["전용면적"] = filtered_df["주택형"].apply(
        lambda x: "".join(filter(lambda y: y.isdigit() or y == ".", x))
    )
