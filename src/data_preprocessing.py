...
def filter_unnecessary_columns(df):
    ...
    unnecessary_columns.extend(
        ["건설업체명_시공사", "사업주체명_시행사", "기사 번호", "주요 토픽"]
    )
...
def type_check(df):
    # 공급세대수:  , 제거 후 타입 변환
    df["공급세대수"] = df["공급세대수"].str.replace(",", "").astype(float)

    # 접수건수: , 제거 후 타입 변환
    df["접수건수"] = df["접수건수"].str.replace(",", "").astype(float)
...
def add_estate_price(df):
    try:
        ...
        # 필요한 컬럼만 유지
        df_estate_price = df_estate_price[
...
def add_market_profit(df, type):
    try:
        ...
        if type == "train":
            ...
            df["전용면적당 시세차익"] = (
                df["전용면적당 공급금액(최고가기준)"] - df["전용면적당 거래금액(만원)"]
            )

        # 불필요한 칼럼 정리 (NaN 방지)
        df.drop(
            columns=["년월", "전용면적당 거래금액(만원)"], inplace=True, errors="ignore"
        )
...
def feature_pre(df, type, target):
    ...
    # 🔸 타겟에 따라 조건적으로 드랍 컬럼 추가
    def get_drop_cols(target_type: str):
        drop_cols = drop_cols_common.copy()

        if target_type == "low":
            drop_cols.append("최고당첨가점")
        elif target_type == "high":
            drop_cols.append("최저당첨가점")
        elif target_type == "gain":
            drop_cols.extend(["최고당첨가점", "최저당첨가점"])
...
def pipeline(type, target):
    ...
    feature_transformer = FunctionTransformer(
        feature_pre, kw_args={"type": type, "target": target}
    )
